...
export async function createCourse({ facultyId, courseCode, courseName, semester }) {
  const { data, error } = await supabase
    .from('courses')
    .insert({ faculty_id: facultyId, course_code: courseCode, course_name: courseName, semester })
    .select()
    .single()
